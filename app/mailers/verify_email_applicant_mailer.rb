...
class VerifyEmailApplicantMailer < ActionMailer::Base
  ...
  def send_applicant_email_verification(applicant, admission = nil)
    ...
    @applicant = applicant
    @admission = admission
    mail(to: applicant.email, subject: t((@admission.present? && @admission.isfit?) ? 'applicants.email_verification.email_subject_isfit' : 'applicants.email_verification.email_subject'))
  end
end
