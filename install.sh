...
### Imports ###
[ -f bash_utils.sh ] && echo "$BOT: Sourcing bash_utils.sh" && . bash_utils.sh
[ -f ~/.bash_profile ] && echo "$BOT: Sourcing ~/.bash_profile" && . ~/.bash_profile
[ -f ~/.bashrc ] && echo "$BOT: Sourcing ~/.bashrc" && . ~/.bashrc
### End: Imports ###


# Initialise global 'X_INTERACTIVE'. Defaults to "y".
X_INTERACTIVE=${X_INTERACTIVE:="y"}
echo "$BOT: Running in interactive mode: $X_INTERACTIVE"
echo
...
echo
do_action "\"I understand\"" "echo '$BOT: Here we go!'; sleep 1;" "y" || eval "echo \"$BOT: That's okay, I can't read either 😔. See ya later!\"; sleep 1; exit"
### End: Welcome ###


### Requirements ###

[ -d Samfundet ] && echo "$BOT: 'Samfundet' already exists in this directory, aborting installation." && exit 1
...
if [ $? == 0 ] && [ `which brew` ]; then
    # Update brew.
    echo "$BOT: Updating and upgrading brew:"
    brew update && brew upgrade && brew upgrade --cask
    echo ; echo "$BOT: Installing gcc:"
...
if [ IS_MAC == 0 ]; then
    ...
    ### google-chrome ###
    echo ; echo ; echo ; echo "==============================================================="
    do_action "$BOT: Install google-chrome (optional)?" "brew install --cask google-chrome" $X_INTERACTIVE

    ### iterm2 ###
    echo ; echo ; echo ; echo "==============================================================="
    echo "Iterm2 is an improved version of Terminal."
    do_action "$BOT: Install iterm2 (optional)?" "brew install --cask iterm2" $X_INTERACTIVE


    ### visual-studio-code ###
    echo ; echo ; echo ; echo "==============================================================="
    do_action "$BOT: Install visual-studio-code (optional)?" "brew install visual-studio-code" $X_INTERACTIVE
fi
...
if [ `which gh` ]; then
    ...
    do_action "$BOT: Add ssh key to your Github account?" "" "y"
    if [ $? == 0 ]; then
        # Foolproof for Sigve :P
        echo
        sleep 1
        echo "$BOT: Log into Github.com, select SSH as preferred method, then use PAT to authenticate."
        sleep 3
        echo
        gh auth login
    fi
    ...
    if [ $? == 0 ]; then
        get_var_with_confirm "KEY_PRIV" "$BOT: Please give me the path to a PRIVATE ssh key: "
...
if [ `ls Samfundet/README.md` ] ; then # Simple check if an arbitrary file exists.
    ...
    #     # echo ; echo ; echo ; echo "==============================================================="
    #     # do_action "Install recommended vscode extensions from .vscode/extensions.recommended.json?" "install_extensions .vscode/extensions.recommended.json" $X_INTERACTIVE
...
echo 
echo
echo "$BOT: You can now run this command to start the project in a docker container:"
